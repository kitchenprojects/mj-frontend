...
function AdminNavLink({ to, icon: Icon, label, isExpanded }) {
  ...
  return (
    <NavLink to={to} end className={navLinkClass}>
      ...
      <span
        className={`ml-4 overflow-hidden whitespace-nowrap transition-all duration-200 ${isExpanded ? 'max-w-xs' : 'max-w-0'
          }`}
      >
        {label}
      </span>
...
export default function AdminLayout() {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      {/* --- Sidebar --- */}
      <aside
        className={`fixed top-0 left-0 h-screen bg-white shadow-xl z-50 flex flex-col transition-all duration-300 ease-in-out ${isExpanded ? 'w-64' : 'w-20'
          }`}
      >
      ...
      <main
        className={`flex-grow p-4 md:p-6 transition-all duration-300 ease-in-out ${isExpanded ? 'ml-64' : 'ml-20'
          }`}
      >
        <Outlet />
      </main>
